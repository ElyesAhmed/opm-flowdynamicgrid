   ...
   std::size_t globalToEclIndex(  std::size_t elem) {
                  ...
                  unsigned timeIdx) const
    {
        ...
        if (nonTrivialBoundaryConditions()) {
            ...
            const auto&entity = context.stencil(timeIdx).entity(interiorDofIdx);
            const auto [type, massrate] = boundaryCondition(container_[entity].preAdaptIndex, indexInInside);
            if (type == BCType::THERMAL)
                values.setThermalFlow(context, spaceIdx, timeIdx, boundaryFluidState(container_[entity].preAdaptIndex, indexInInside));
            else if (type == BCType::FREE || type == BCType::DIRICHLET)
                values.setFreeFlow(context, spaceIdx, timeIdx, boundaryFluidState(container_[entity].preAdaptIndex, indexInInside));
                ...
                unsigned timeIdx) const
    {
        ...
        for (unsigned eqIdx = 0; eqIdx < numEq; ++ eqIdx) {
            rate[eqIdx] /= this->model().dofTotalVolume(globalDofIdxCurrent);
            ...
            if constexpr (enableEnergy) {
                for (unsigned i = 0; i < phidx_map.size(); ++i) {
                    ...
                    if (source.hasHrate({ijk, sourceComp})) {
                        ...
                        Scalar mass_rate = source.rate({ijk, sourceComp})/ this->dofTotalVolumeOrg(globalDofIdx)*this->model().dofTotalVolume.dofTotalVolume(globalDofIdxCurrent)/ this->dofTotalVolumeOrg(globalDofIdx);
